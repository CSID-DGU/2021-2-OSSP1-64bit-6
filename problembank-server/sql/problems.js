module.exports = {
    ...
    //==================HEATMAP====================
    countTime : "select DATE_FORMAT(DATE_ADD(U.submit_time, INTERVAL 16 HOUR), '%Y/%m/%d') AS submit_time2, SUM(cnt) AS cnt FROM(select DATE_FORMAT(DATE_ADD(S.timestamp, INTERVAL 16 HOUR), '%Y/%m/%d') AS submit_time, COUNT(*) AS cnt from plass_problem_multiplechoice_submit S left outer join plass_problem_multiplechoice_category C ON S.problem_id = C.problem_id left outer join plass_problem_multiplechoice P ON S.problem_id = P.id WHERE S.user_id=1 AND S.answer_status='true' GROUP BY DATE_FORMAT(DATE_ADD(S.timestamp, INTERVAL 16 HOUR), '%Y/%m/%d') UNION all select DATE_FORMAT(DATE_ADD(S.timstamp, INTERVAL 16 HOUR), '%Y/%m/%d') AS submit_time, COUNT(*) AS cnt from plass_problem_shortans_submit S left outer join plass_problem_shortanswer_category C ON S.problem_id = C.problem_id left outer join plass_problem_shortans P ON S.problem_id = P.id WHERE S.user_id=1 AND S.answer_status=1 GROUP BY DATE_FORMAT(DATE_ADD(S.timstamp, INTERVAL 16 HOUR), '%Y/%m/%d'))AS U GROUP BY DATE_FORMAT(DATE_ADD(U.submit_time, INTERVAL 16 HOUR), '%Y/%m/%d')",
    conTime : "SELECT MAX(con_days) AS maxcon FROM(SELECT COUNT(*) AS con_days FROM(SELECT BB.*, BB.RNUM+BB.diff_day AS CON_DAY	FROM(SELECT AA.* , @rownum := @rownum+1 AS RNUM, TIMESTAMPDIFF(DAY, submit_time2, '20211103') AS diff_day FROM(select DATE_FORMAT(DATE_ADD(UU.submit_time, INTERVAL 16 HOUR), '%Y/%m/%d') AS submit_time2, SUM(cnt) AS cnt FROM(select DATE_FORMAT(DATE_ADD(S.timestamp, INTERVAL 16 HOUR), '%Y/%m/%d') AS submit_time, COUNT(*) AS cnt from plass_problem_multiplechoice_submit S left outer join plass_problem_multiplechoice_category C ON S.problem_id = C.problem_id	left outer join plass_problem_multiplechoice P ON S.problem_id = P.id WHERE S.user_id=1 AND S.answer_status='true' GROUP BY DATE_FORMAT(DATE_ADD(S.timestamp, INTERVAL 16 HOUR), '%Y/%m/%d') UNION all select DATE_FORMAT(DATE_ADD(S.timstamp, INTERVAL 16 HOUR), '%Y/%m/%d') AS submit_time, COUNT(*) AS cnt	from plass_problem_shortans_submit S left outer join plass_problem_shortanswer_category C ON S.problem_id = C.problem_id left outer join plass_problem_shortans P ON S.problem_id = P.id WHERE S.user_id=1 AND S.answer_status=1 GROUP BY DATE_FORMAT(DATE_ADD(S.timstamp, INTERVAL 16 HOUR), '%Y/%m/%d'))AS UU GROUP BY DATE_FORMAT(DATE_ADD(UU.submit_time, INTERVAL 16 HOUR), '%Y/%m/%d')) AS AA, (SELECT @rownum := 0) R) AS BB)AS CC GROUP BY CON_DAY ORDER BY con_days DESC)AS DD",
}
