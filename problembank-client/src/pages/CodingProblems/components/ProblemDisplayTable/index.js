			...
			<tbody>
				{
					problems.map((item, index) => {
						return (
							<tr key = {index} onClick={() => props.history.push(`/codeproblems/view?id=${item.id}`)}>
								<th>{item.id}</th>
								<td>{item.name}</td>
								<th>{item.level}</th>
								<th>{moment(item.created).format('YYYY-MM-DD')}</th>
								<td>{item.tagInfo.parent_name} - {item.tagInfo.name}</td>
								<th style={item.submit.length !== 0 ? item.submit.answer_status === 1 ? {color: 'blue'} : {color: 'red'} : {}}>{item.submit.length !== 0 ? item.submit.answer_status === 1 ? '성공' : '실패' : '' }</th>
							</tr>
